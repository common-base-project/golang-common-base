...
func Load(g *gin.Engine) {
	...
	config := cors.Config{
		...
		AllowHeaders:     []string{"Origin", "Content-Length", "Content-Type", "Authorization", "X-Requested-With", "Request-Start"},
		AllowOrigins:     []string{"*"},
	...
	g.GET(fmt.Sprintf("%s%s", viper.GetString(`api.version`), "/health"), func(c *gin.Context) {
