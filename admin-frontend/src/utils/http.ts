...
http.interceptors.response.use(
  (response) => {
    ...
    if (typeof data.code === "undefined" && typeof data.errno !== "undefined") {
    ...
    // 错误提示
    ElMessage.error(response.data.msg || response.data.errmsg || "请求失败");
    ...
    return Promise.reject(new Error(response.data.msg || response.data.errmsg || "Error"));
  },
  (error) => {
